...
export default function CommunityPortal({ initialCommunity }: { initialCommunity?: string }) {
  ...
  const moveToCommunity = (event: React.MouseEvent<HTMLAnchorElement>, path: string) => {
    event.preventDefault();
    window.location.assign(path);
  };
  ...
  return <main className="hot-shell" id="top">
    ...
    <nav className="hot-community-tabs" aria-label="커뮤니티 선택"><Link className={community === "all" ? "active" : ""} href="/" onClick={(event) => moveToCommunity(event, "/")}>종합</Link>{communities.map((item) => { const path = `/community/${item.slug}`; return <a key={item.slug} className={community === item.slug ? "active" : ""} href={path} onClick={(event) => moveToCommunity(event, path)}>{item.name.replace("드림", "")}</a>; })}</nav>
