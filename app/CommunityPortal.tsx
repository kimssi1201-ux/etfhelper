...
export default function CommunityPortal({ initialCommunity }: { initialCommunity?: string }) {
  ...
  return <main className="hot-shell" id="top">
    ...
    <nav className="hot-community-tabs" aria-label="커뮤니티 선택"><a className={community === "all" ? "active" : ""} href="/">종합</a>{communities.map((item) => <a key={item.slug} className={community === item.slug ? "active" : ""} href={`/community/${item.slug}`}>{item.name.replace("드림", "")}</a>)}</nav>
