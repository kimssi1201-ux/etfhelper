...
export default function CommunityPortal({ initialCommunity }: { initialCommunity?: string }) {
  ...
  return <main className="hot-shell" id="top">
    ...
    <section className="hot-list" aria-label="인기 게시글">{result.posts.map((post, index) => <HotPost key={post.id} post={post} rank={index + 1} />)}{!result.posts.length && <div className="hot-empty">검색 결과가 없습니다.</div>}</section>
